...
def run_training():
    ...
    train_data_path = os.path.join(data_config['processed_data_dir'], data_config['train_filename'])
    ...
    features_tensor = torch.tensor(processed_df[['demand_scaled', 'demand_diff']].values, dtype=torch.float32)

    # 4. Train the Model
    with mlflow.start_run(run_name=f"{train_config['mlflow']['model_name']}_training") as run:
        ...
        mlflow.log_artifact(scaler_path, artifact_path=train_config['artifacts']['scaler_path'])

        # Log the PyTorch model using the wrapper
        mlflow.pyfunc.log_model(
            artifact_path=train_config['artifacts']['model_path'],
            python_model=LSTMWrapper(trained_model, processor), # Pass processor to wrapper
