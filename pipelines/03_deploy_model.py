...
def prepare_inference_model():
    ...
    print(
        f"Fetching model with alias '{deploy_config['model_alias']}' from: {model_uri}"
    )
    ...
    try:
        ...
        print(
            f"Please ensure a model named '{model_name}' has a version with the alias '{deploy_config['model_alias']}'."
        )
        sys.exit(1)
